...
@onready var frog_jump = $FrogJump


const JUMP_VELOCTY: Vector2 = Vector2(100, -159)
...
func apply_jump() -> void:
	...
	velocity = JUMP_VELOCTY
	
	if animated_sprite_2d.flip_h == false:
		velocity.x = velocity.x * -1
		
	_jump = false
	SoundManager.play_clip(frog_jump, SoundManager.SOUND_FROG_JUMP)
