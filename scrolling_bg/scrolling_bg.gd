...
@export var sprite_offset: Vector2 = Vector2( 0, -470 )
...
func add_backgrounds() -> void:
	...
	var time_offset = increment
	var files_list = BG_FILES[level_number]
	
	for index in range(files_list.size()):
		var background_file = files_list[index]
		
		if index == 0:
			add_layer(background_file, 1)
		else:
			add_layer(background_file, time_offset)
